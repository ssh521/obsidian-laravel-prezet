...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    >
        ...
        >
            <div class="z-10 p-20 text-left">
                <div class="text-8xl font-bold text-gray-200">
                    {{ $fm->title }}    
                </div>
                
                @if ($fm->category)
                    <div class="mt-2 text-sm font-bold text-gray-200">
                        {{ $fm->category }}
                    </div>
                @endif

                <div class="mt-2 text-sm font-bold text-gray-200">
                    작성일 : {{ $fm->createdAt->format('Y-m-d') }} / 수정일 : {{ $fm->updatedAt->format('Y-m-d') }}
                </div>              
                <div class="mt-6 text-2xl font-bold text-gray-200">
                    {{ $fm->excerpt }}
                </div>

                @if ($fm->tags)
                    <div class="mt-4 text-2xl font-bold text-gray-200">
                        @foreach ($fm->tags as $tag)
                            <span class="text-sm font-bold text-white rounded-md bg-gray-800 px-2 py-1 mr-2">
                            {{ $tag }}
                            </span>
                        @endforeach
                    </div>
